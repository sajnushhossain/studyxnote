<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-black text-slate-900 tracking-tight font-outfit">New Repository Entry</h1>
                <p class="text-slate-400 font-medium mt-1 italic uppercase tracking-widest text-[10px]">Create a new academic resource</p>
            </div>
    ...
    <div class="max-w-3xl" x-data="{ type: '{{ old('type', 'text') }}', showNewCategory: {{ old('new_category_name') ? 'true' : 'false' }} }">
        @if ($errors->has('error'))
        ...
        <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-xl p-10">
            <form action="{{ route('admin.notes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="input-field" placeholder="e.g. Advanced Calculus: Integrals" required>
                ...
                <div x-show="type === 'text'" class="space-y-2">
                    ...
                    <textarea name="content" rows="10" class="input-field font-medium leading-relaxed" placeholder="Detailed notes go here...">{{ old('content') }}</textarea>
                    <x-input-error :messages="$errors->get('content')" class="mt-2" />
                </div>

                <!-- Image Upload -->
                <div x-show="type === 'image'" class="space-y-2" x-cloak>
                    ...
                    <div class="relative group">
                        ...
                        <div class="input-field flex items-center justify-center border-dashed border-2 py-12 group-hover:border-indigo-400 transition-colors">
                            <div class="text-center">
                                ...
                                <p class="text-sm font-bold text-slate-400 group-hover:text-slate-600 transition-colors">Click to upload or drag & drop</p>
                ...
                <div class="pt-6 border-t border-slate-50 flex gap-4">
                    ...
                    <button type="reset" class="btn btn-secondary px-8 py-4 text-sm tracking-[0.2em]">
                        Clear
                    </button>
                </div>
            </form>
